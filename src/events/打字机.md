...
@[type_parallel] {
    ...
    "offset": [0, 0],
    "label_settings": {"c": {"text_color": "#ffffff"}}
}
```
...
@[type_parallel] {
    ...
    "label_settings": {"c": {"text_color": "#ffffff"}}    # 标签独立样式设置
}
```
缩写:`@[type]`
...
- **`label_settings`**：对象，用于为文本中的特定标签设置独立的样式与音效。
  ...
  - 理论支持修改打字机里的所有全局参数
  - 据此反推出打字机也有一个`text_color`参数可以直接修改所有字的颜色
