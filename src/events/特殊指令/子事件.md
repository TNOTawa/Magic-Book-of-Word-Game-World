...
当块内存在 `@[backspace_command]` 时，会阻止玩家本次直接删字，并转由物件自身继续执行块内后续指令。
...
### 拆字触发块
`@[split_command]` 与 `@[end_split_command]` 成对出现。
...
用于处理“玩家对该物件执行拆字”这一触发场景。
当块内存在 `@[split_command]` 时，会阻止玩家本次直接拆字，并转由物件自身继续执行块内后续指令。

但目前还没有可用的@[split_event]指令，所以无法让物件自己拆自己